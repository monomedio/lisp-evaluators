...
(check-equal? (eva '(checking a list)) '(checking a list))
; A function call
(check-equal? (eva '((λ (x) x) 324)) 324)
(check-equal? (eva '((λ (x) x) "say this")) "say this")
(check-equal? (eva '(((λ (z) (λ (y) x)) 324) 325)) 'x)
; A function call whose evaluation of the transformed body is a closure, thus returns a closure.
(check-equal? (eva '(((λ (z) (λ (x) z)) "hi") "not this")) '(λ (x) "hi"))
(check-equal? (eva '(((λ (x) (λ (y) x)) 324) 325)) '(λ (y) 324))
; Substitutions in function calls respect scope
(check-equal? (eva '((λ (y) (λ (y) (λ (y) y))) 324)) '(λ (y) (λ (y) y)))
(check-equal? (eva '(((λ (x) (λ (y) (λ (x) y))) 324) 325)) '(λ (x) 325))
(check-equal? (eva '(((λ (x) (λ (y) (λ (x) "hello"))) 324) 325)) '(λ (x) "hello"))
(check-equal? (eva '(((((λ (x) (λ (y) (λ (x) (λ (z) z)))) 373) 369) 324) 325)) 325)
; General test of eva behaviour
(check-equal? (eva '((λ (x) ((λ (y) x) "jon")) "sam")) '(λ (y) "sam"))
(check-equal? (eva '((λ (x) ((λ (y) ((λ (z) z) "avi")) "jon")) "sam")) "avi")

; Testing substitute
(check-equal? (substitute '(λ (x) x) 324) 324)
(check-equal? (substitute '((λ (z) (λ (x) z)) 369) 324) '((λ (z) (λ (x) z)) 369))
(check-equal? (substitute '(λ (x) "Hi") 324) '(λ (x) "Hi"))
(check-equal? (substitute '(λ (x) ((λ (y) y) "why")) 324) '((λ (y) y) "why"))
